...
describe VLCTechHub::API::V1::Routes do
  ...
  describe 'GET /v1/events' do
    ...
    it 'returns a list of events for that year and month' do
      ...
      expect(last_response).to be_ok
      expect(events_for_year_month(2_016, 0o2)).not_to be_empty
    end
  ...
  describe 'GET /v1/events/publish/' do
    ...
    context 'when event is found' do
      let(:repo) { instance_double(VLCTechHub::Event::Repository, publish: true, find_by_uuid: event) }
      let(:event) { { 'date' => DateTime.now } }

      before { Grape::Endpoint.before_each { |endpoint| allow(endpoint).to receive(:events).and_return(repo) } }

      after { Grape::Endpoint.before_each nil }

      it 'tweets and sends a mail about it' do
        mailer = class_spy(VLCTechHub::Event::Mailer).as_stubbed_const
        twitter = class_spy(VLCTechHub::Event::Twitter).as_stubbed_const

        get '/v1/events/publish/found-id'

        expect(last_response).to be_ok

        expect(mailer).to have_received(:broadcast)
        expect(twitter).to have_received(:new_event)
      end
    end
  end
